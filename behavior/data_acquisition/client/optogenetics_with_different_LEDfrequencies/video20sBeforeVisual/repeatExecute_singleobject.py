...
mydict = {'1.pufffreq':[1.0], '2.LEDfreq':[0.0,1.0,3.0,4.0,25.0]}
...
# set color
print('setting color..')
command = 'python setcolorON.py --host ' + str(args['host']) + ' --acclimation 0 -p False --pufffreq 0 --postPuffInterval 0 --genotype setcolor --width 1 --height 1 --ID 0 --trial 0 --AngularPosBias 0'
os.system(command)
...
os.system('python LRbiasEstimate.py --host '+args['host']+ ' --acclimation 0 --genotype ' + args['genotype'] + ' --ID ' + str(args['ID']) + ' --LEDfreq 0.0 --pufffreq 0.0')
...
for i in range(len(schedule)):
    ...
    command = 'python singleobject_closed.py --host ' + str(args['host']) + ' --acclimation '+ str(args['acclimation']) + ' -p '+puff+' --pufffreq '+str(schedule[i][0])+' --LEDfreq ' + str(schedule[i][1]) + ' --postPuffInterval ' + str(args['postPuffInterval']) + ' --genotype ' + args['genotype'] + ' --width 8 --height 8 --ID ' + str(args['ID']) + ' --trial ' + str(i) + ' --AngularPosBias " ' + str(AngularPosBias)+'"' # negative value would be mistakenly recognized as flags if not given in a " -XXX" format

    print('\n\nexecuting: '+ command)
    os.system(command)

# reverting the color
command = 'python setcolorOFF.py --host ' + str(args['host']) + ' --acclimation 0 -p False --pufffreq 0 --postPuffInterval 0 --genotype setcolor --width 1 --height 1 --ID 0 --trial 0 --AngularPosBias 0'
os.system(command)
print('\n\n all done!')
