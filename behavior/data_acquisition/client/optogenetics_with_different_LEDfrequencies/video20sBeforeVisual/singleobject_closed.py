...
class SingleSquare4Closed(Client):
    ...
    def main(self):
        ...
        print('\n updating LEDfreq of arduino ('+self.arduino.LEDboard+')...\n')
        if self.args['LEDfreq'] == 0.0:
            LEDboutlen = 0
        else:
            LEDboutlen = int(1000.0 / self.args['LEDfreq'] / 2.0)
        os.system("sed -i 's/delay(mydelay)/delay("+str(LEDboutlen)+")/g' ../arduino_LED/arduino_LED.ino")
        print('/home/masato/bin/arduino/arduino --upload --port '+self.arduino.LEDboard+' ../arduino_LED/arduino_LED.ino')
        os.system('/home/masato/bin/arduino/arduino --upload --port '+self.arduino.LEDboard+' ../arduino_LED/arduino_LED.ino')
        ...
        time.sleep(self.baseduration)

        # puff & LED
        print('applying puff and LED...')
        if self.args['pufffreq'] > 0.0:
            self.arduino.switchPuff("ON")
        if self.args['LEDfreq'] > 0.0:
            self.arduino.switchLED("ON")
        time.sleep(self.puffduration-0.05) # due to slight delay in serial com, puff valve ends up left open unless 'OFF' signal is sent a bit earlier
        if self.args['pufffreq'] > 0.0:
        ...
        os.system("sed -i 's/delay("+str(LEDboutlen)+")/delay(mydelay)/g' ../arduino_LED/arduino_LED.ino")
        print('all done')
