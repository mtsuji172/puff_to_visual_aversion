...
# import/export
import('../data/VGluTTk_activation_visual/export/')

# pose estimation
loadData('../data/VGluTTk_activation_visual/predictions/', '../data/VGluTTk_activation_visual/statistics/')

# calc indices for all files
main_batch('../data/VGluTTk_activation_visual/ballrecordings/', parallel=T)
...
puffeffect$genotype = factor(puffeffect$genotype, levels=c('UASstopCsChrimson-VGluTlexA.-lexAopFLP.Tk2G4-ATRnega','UASstopCsChrimson-VGluTlexA.-lexAopFLP.Tk2G4'))
...
puffeffect_mean$genotype = factor(puffeffect_mean$genotype, levels=(unique(puffeffect_mean$genotype)))
ggplot(subset(puffeffect_mean, uniq%in%uniq.myset), aes(genotype, index_onset, color=genotype, fill=genotype)) +
	labs(x='',y='') +
    theme_classic() +
    theme(text=element_text(size=25), legend.position='None') +
    geom_hline(yintercept=0, lty=2) +
...
t.test(index_onset~genotype, subset(puffeffect_mean, uniq%in%uniq.myset))
t.test(subset(puffeffect, LEDfreq==25 & genotype=='UASCsChrimson.-Tk2G4.' & uniq%in%uniq.myset)$index_onset)
t.test(index_onset~genotype, subset(puffeffect_mean, genotype!='UASCsChrimson.-DTk1.DTk2Tk2G4.' & uniq%in%uniq.myset))
t.test(index_onset~genotype, subset(puffeffect_mean, genotype!='UASCsChrimson.-Tk2G4.-ATRnega' & uniq%in%uniq.myset))
...
pose_flymean.sub = subset(pose_flymean, trialtype=='normal' & uniq%in%uniq.myset & label%in%c('glooming','stop','walk') & uniq%in%puffeffect$uniq)
pose_flymean.sub = subset(pose_flymean, trialtype=='normal' & uniq%in%uniq.myset & label%in%c('glooming','stop','walk') & LEDfreq%in%c(0,25) & uniq%in%subset(puffeffect, LEDfreq==25)$uniq)
