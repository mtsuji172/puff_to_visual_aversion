            ...
            if(nrow(subpose_puffeffect)>0){
                out = rbind(out,
                            data.frame(genotype=g,
                                       label=i,
                                       pvalue=t.test(subpose_puffeffect$ratio)$p.value))
            } else{
                print(paste0("error in label: ", i, " genotype: ", g))
            }
        }
    }
    out$pvalue = p.adjust(out$pvalue, method=p.adjust.method)
    return(out)
}
        ...
        lapply(1:length(label.set), function(l){
            ...
            out =
                lapply(1:length(genotype.set), function(g){
                    subsubdata = subset(subdata.l, genotype%in%c(control,genotype.set[g]%>%as.character))
                    data.frame(label=label.set[l],
                               genotype=genotype.set[g],
                               pvalue = t.test(ratio~genotype, data=subsubdata)$p.value)
                }) %>% bind_rows
            out$pvalue = p.adjust(out$pvalue, method=p.adjust.method)
            out
        }) %>% bind_rows
