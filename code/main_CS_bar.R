...
### stat
npuff.set = unique(result_all_mean$npuff)
stat.out = 
    lapply(1:length(npuff.set), function(k){
               result_all_mean.k =
                   subset(result_all_mean, npuff==npuff.set[k])
               data.frame(npuff = npuff.set[k],
                            p.value = t.test(-result_all_mean.k$index_throughout,
                                             alternative = 'greater')$p.value)
            }) %>% bind_rows
stat.out$p.value = p.adjust(stat.out$p.value, method='bonferroni')
print(stat.out)
write.csv(stat.out, paste0('./CS_bar/result_all_mean_activityThresh',activity_thresh,'stat.csv'), row.names=F)
write.csv(stat.out, './CS_bar/result_all_mean_baseAttracted_stat.csv', row.names=F)

### stat
stat.out = data.frame(p.value = t.test(-puffeffect$index_throughout)$p.value)
print(stat.out)
write.csv(stat.out, './CS_bar/puffeffect_stat.csv', row.names=F)
write.csv(stat.out, './CS_bar/puffeffect_stopThresh_baseAttracted_stat.csv', row.names=F)
